...
const { getLastSegment, isNumeric } = require("./utils");
...
class Json2iob {
  ...
  constructor(adapter, alreadyCreatedObjects = {}) {
    ...
    this.timeKey = ["lastActive", "lastUpdate"];
    this.noInfoDP = [
      "eventsQueue",
      "rfRegions",
  ...
  async parse(path, element, options = { write: false }) {
    ...
    if (typeof element === "object" && element?.productLabel) {
      if (options.rewriteDP) {
        await this.adapter.setObjectAsync(nodeId, {
          type: "device",
          common: {
            name: element.name || element.manufacturer,
            statusStates: {
    ...
    if (
      ...
      if (element?.commandClass) {
        ...
        if (this.RGB.includes(element.propertyKeyName)) {
          parsePath = utils.replaceLastDot(parsePath);
        }
      }

      if (options.rewriteDP) {
        delete this.alreadyCreatedObjects[parsePath];
      }
      ...
      if (!this.alreadyCreatedObjects[parsePath]) {
        try {
          ...
          if (options.rewriteDP) {
            await this.adapter.setObjectAsync(parsePath, {
              type: "state",
              common,
              native: {
    ...
    for (const key of Object.keys(element)) {
      ...
      if (key == "ready" || key == "status") {
        fullPath = fullPath.replace(".info.", ".");
      }

      if (options.rewriteDP) {
        delete this.alreadyCreatedObjects[fullPath];
      }
      ...
      if (!this.alreadyCreatedObjects[fullPath]) {
        ...
        if (options.rewriteDP) {
          await this.adapter.setObjectAsync(fullPath, {
            type: "state",
            common,
            native: {
  ...
  getRole(element, options, dpName) {
    ...
    if (this.timeKey.includes(dpName)) {
  ...
  resolveCommandClassValue(element) {
    ...
    if (type === "number") {
      if (element?.value) {
        return isNumeric(element.value) ? element.value : 0;
      }
   ...
   * @param state
   */
  createZ2mMessage(id, state) {
    const idCorr = id?.startsWith("zwave2mqtt.")
      ? id.slice("zwave2mqtt.".length)
      : id;
